...
const advancedAnalysis = (graph, patterns) => {
    ...
    if (patterns.hasCache)
        scalabilityScore += 2;
    if (!patterns.isFullySync)
        scalabilityScore += 2;
    if (!patterns.godService)
        scalabilityScore += 1;
    let trafficCapacity = "Low";
    if (scalabilityScore >= 5)
        trafficCapacity = "High (100k+ req/min)";
    else if (scalabilityScore >= 3)
        trafficCapacity = "Medium (10k req/min)";
    else
        trafficCapacity = "Low (<5k req/min)";
    // =========================================

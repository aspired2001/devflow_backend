...
export const buildGraph = (elements: any[]): Graph => {
    ...
    for (const el of elements) {
        ...
        if (!archType || typeof archType !== "string") continue
        if (archType.toLowerCase().includes("label")) continue
    ...
    for (const el of elements) {
        ...
        const edgeType =
            el.customData?.edgeType || "sync" // future: async, read, write
